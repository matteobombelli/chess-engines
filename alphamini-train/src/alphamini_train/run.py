...
from .cuda_runtime import recorded_runtime_identity
from .errors import ConfigError, IntegrityError

STATE_SCHEMA = "alphamini.run-state.v1"
RUN_SCHEMA = "alphamini.run-manifest.v1"
EXTENSION_SCHEMA = "alphamini.budget-extension.v1"
BUDGET_MILESTONE_SCHEMA = "alphamini.budget-milestone.v1"
...
def git_identity(worktree: Path) -> dict[str, Any]:
    ...
    try:
        ...
        digest = hashlib.sha256(b"alphamini-worktree-content-v1\0")
...
def runtime_identity() -> dict[str, Any]:
    ...
    result["alphamini_cuda_runtime"] = recorded_runtime_identity()
    try:
...
class RunRepository:
        ...
        *,
        ...
        manifest = {
            ...
            "locks": {
                ...
                "uv_lock_sha256": (
                    sha256_file(worktree / "alphamini-train" / "uv.lock")
                    if (worktree / "alphamini-train" / "uv.lock").is_file()
                    else None
                ),
            },
            "parent": parent,
    ...
    def open(cls, root: Path | str) -> tuple["RunRepository", ResolvedConfig]:
        ...
        if not (repository.root / "RUN.json").is_file():
            raise IntegrityError(f"not an AlphaMini run: {repository.root}")
...
def safe_budget_boundary(state: dict[str, Any]) -> bool:
    """A fully promoted model boundary at which training may stop."""

    return state.get("phase") == "ready_collect"
...
def attach_initial_budget_milestone(
    ...
    milestone = {
        ...
        "game_id_next": int(state["game_id_next"]),
        "current_checkpoint": copy.deepcopy(state.get("current_checkpoint")),
        "current_model": copy.deepcopy(state.get("current_model")),
...
def read_initial_budget_milestone(
    ...
    if (
        ...
        or milestone.get("phase") != "ready_collect"
...
class ActiveSession:
    ...
    def __init__(self, repository: RunRepository, command: str, heartbeat_seconds: int):
        ...
        self._thread = threading.Thread(
            ...
            name=f"alphamini-heartbeat-{self.session_id[:8]}",
            daemon=True,
        )
        self._thread.start()
    ...
    def _write(self, elapsed: float) -> None:
        atomic_write_json(
            ...
            {
                "schema": "alphamini.active-session.v1",
...
def recover_interrupted(repository: RunRepository, *, force: bool = False) -> dict[str, Any]:
    ...
    if state.get("phase") == "ready_collect":
        quarantine_path(repository.root / "cycles" / f"cycle-{cycle_id:06d}" / "collection")
    elif state.get("phase") == "ready_materialize":
        quarantine_path(repository.root / "cache" / f"cycle-{cycle_id:06d}")
    # A final filename is never selected by mtime. Unsealed partials are preserved
    # under quarantine and deterministic IDs regenerate their work.
...
def extend_budget(repository: RunRepository, seconds: float, reason: str) -> dict[str, Any]:
    ...
    if not safe_budget_boundary(state):
        raise ConfigError("finish the in-flight cycle at its safe boundary before extending")
    state = attach_initial_budget_milestone(repository, state)
...
def fork_run(
    ...
    *,
    ...
    checkpoint = source_state.get("current_checkpoint")
    parent = {
